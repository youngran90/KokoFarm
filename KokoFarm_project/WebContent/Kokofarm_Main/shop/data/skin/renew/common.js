...
function chkForm(form)
{
	...
	for (i=0;i<form.elements.length;i++){
		...
		if (currEl.getAttribute("option")!=null && currEl.value.length>0){
			if (currEl.getAttribute("option")=="regPass" && !chkLength(currEl,10)) return false;
...
/* 상품재입고 알림 팝업 띄우기 2016.09.20 양경진추가 */
function popup_js(goodsno,width,height,s){
  window.open('./popup_request_stocked_noti.php?goodsno='+goodsno,'','width='+width+',height='+height+',scrollbars='+s);
}
/* 상품재입고 알림 팝업 띄우기 끝 */
...
function popup_card_rec(src,width,height) {

	window.open(src,'popupIssue','width='+width+',height='+height+',toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes');
}

function popup_resize(src,width,height)
{
...
function cartAdd(form,redirectType,goodsnm)
{
	var opt_cnt = 0, data;
	...
	// GA 삽입
	ga('send', 'event', '상품상세페이지','장바구니', goodsnm);
...
function SameAddressSub(text) {
	...
	if(div_road_address.innerHTML.trim() == "") {
...
// 오픈팝업 공유버튼 눌렀을때 발생하는 스크립트 2016.08.03 양경진추가
function ajaxHitis(cate,m_id,type){
  $.post('/shop/hit_check.php',{'cate':cate,'m_id':m_id,'type':type}, function(res,status) {
...
function checkHit(cate,link,target,width,height,m_id,type) {
  var url = link;
  var t = target; //새창인지 팝업인지 구분
  var w = width; //팝업시 팝업넓이
  var h = height; //팝업시 팝업높이
  var name = "win_" + type; //윈도우명
  ajaxHitis(cate,m_id,type); 
  if(t == "_blank") {
	window.open(url);
  }else if(t == "pop"){
	window.open(url,name,'width='+w+',height='+h+',scrollbars=1');
  }else{
	location.href = url;
  }
}
